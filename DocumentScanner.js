...
export default class DocumentScanner {
	...
	scanImage(src, addOutline = false) {
		...
		if(contour)
			this.scaleContour(contour, 1/scale);

		// crop and transform the detected document
		const img = contour ? this.fourPointTransform(src, contour) : null;

		// draw the contour
		if(addOutline && contour)
			this.cv.drawContours(src, this.arrayToMatVector([contour]), -1, new this.cv.Scalar(0, 255, 0, 255), 3, this.cv.LINE_8, hierarchy, 0);
